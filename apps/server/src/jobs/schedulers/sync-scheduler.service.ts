...
import { ORDER_SYNC_QUEUE } from '../processors/order-sync.processor';
...
export class SyncSchedulerService implements OnModuleInit {
  ...
  constructor(
    ...
    @InjectQueue(ORDER_SYNC_QUEUE) private orderSyncQueue: Queue,
  ...
  async registerScheduledJobs() {
    ...
    for (const store of stores) {
      ...
      // Returns sync — every 4 hours
      await this.returnsSyncQueue.add(
        `returns-sync-${store.id}`,
        { storeAccountId: store.id },
        {
          ...jobOptions,
          repeat: {
  ...
  async triggerReturnsSync(storeAccountId: string) {
    ...
    this.logger.log(`Triggered manual returns sync job ${job.id} for store ${storeAccountId}`);
    return job;
  }

  async triggerOrderSync(storeAccountId: string) {
    const job = await this.orderSyncQueue.add(
      `manual-order-sync-${storeAccountId}`,
      { storeAccountId },
      {
        removeOnComplete: { count: 10 },
        removeOnFail: { count: 20 },
    ...
    this.logger.log(`Triggered manual order sync job ${job.id} for store ${storeAccountId}`);
    return job;
  }
  ...
  async getSyncStatus(storeAccountId: string) {
    const [productJobs, stockJobs, promoJobs, financeJobs, returnsJobs, ratingJobs, orderJobs] = await Promise.all([
      ...
      this.orderSyncQueue.getJobs(['active', 'waiting', 'delayed']),
    ]);
  ...
  private async cleanRepeatableJobs() {
    const queues = [this.productSyncQueue, this.promotionSyncQueue, this.stockSyncQueue, this.financeSyncQueue, this.returnsSyncQueue, this.ratingSyncQueue, this.orderSyncQueue];
